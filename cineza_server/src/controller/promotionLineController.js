...
const createPromotionLineController = async (req, res) => {
    const { code, startDay, endDay, promotionLineStatus, typePromotion, promotionHeaderCode } = req.body;
    try {
        const checkCode = await getPromotionLineByCodeService(code);
        if (checkCode == null) {
            const newPromotion = await createPromotionLineService({ code, startDay, endDay, promotionLineStatus, typePromotion, promotionHeaderCode });
