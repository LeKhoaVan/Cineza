...
const getTotalPriceByService = async (codeOrder) => {
    ...
    const [total, metadata] = await db.sequelize.query(query, { type: QueryTypes.SELECT, raw: false });
    return total
}
...
const getOrderByUserService = async (codeUser) => {
    ...
    const dataOrders = await db.sequelize.query(query, { type: QueryTypes.SELECT });
    return dataOrders;
}

module.exports = { createOrderService, getOrderServiceByCode, getOrderByUserService, getTotalPriceByService };